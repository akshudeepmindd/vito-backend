...
const register = async (req, res, next) => {
  try {
    ...
    responseHelper.success(
      res,
      `user created successfully a verification link send to ${userMail}`,
      200
    );
  } catch (error) {
    next(error);
...
const resendToken = async (req, res, next) => {
  try {
    const userToken = await userValidator.resendVerificationToken.validateAsync(
      req.body
    );
